...
const (
	...
	maxDomainLength = 100               // Threshold for suspicious domain length
	beaconInterval  = 10 * time.Second  // Interval to check for beaconing
	beaconThreshold = 5                 // Threshold for beaconing detection
)
...
func processPacket(packet gopacket.Packet) {
	...
	for _, query := range dns.Questions {
		...
		if len(domain) > maxDomainLength {
			fmt.Printf("[ALERT] Suspiciously long domain detected: %s\n", domain)
		}
